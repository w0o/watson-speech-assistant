...
/* global ConversationApi: true, Common: true, SpeechApi: true */
...
var ConversationPanel = (function () {
  ...
  function init() {
    chatUpdateSetup();
    ConversationApi.sendRequest('', null);
    setupInputBox();
  }

  function onRecordingStopped() {
    document.querySelector(settings.selectors.speechIndicator).style = '';
  }

  function onRecordingStarted() {
    document.querySelector(settings.selectors.speechIndicator).style = 'color: red;';
  }

  // Set up callbacks on payload setters in ConversationApi module
  // This causes the displayMessage function to be called when messages are sent / received
  function chatUpdateSetup() {
    var currentRequestPayloadSetter = ConversationApi.setRequestPayload;
    ConversationApi.setRequestPayload = function (newPayloadStr) {
      currentRequestPayloadSetter.call(ConversationApi, newPayloadStr);
      displayMessage(JSON.parse(newPayloadStr), settings.authorTypes.user);
    };

    var currentResponsePayloadSetter = ConversationApi.setResponsePayload;
    ConversationApi.setResponsePayload = function (newPayloadStr) {
      currentResponsePayloadSetter.call(ConversationApi, newPayloadStr);
      // TODO: Add TTS here
  ...
  function setupInputBox() {
    ...
    // setup speech input button
    document.querySelector(settings.selectors.speakButton).addEventListener('click', SpeechApi.toggleMicRecording);
  ...
  function addChatVisualElement(chatBoxElement, res, isUser, isTop) {
    var currentDiv = getDivObject(res, isUser, isTop);
    if (!isUser) {
      var textMessage = res.innerhtml;
      // speak out the message
      if (res.type === 'image') {
        textMessage = currentDiv.textContent || currentDiv.innerText || '';
      }
      SpeechApi.speak(textMessage).then((audio) => {
  ...
  function setResponse(responses, isUser, chatBoxElement, index, isTop) {
    if (index < responses.length) {
      console.log(`Displaying response ${JSON.stringify(responses[index])}`);
      var res = responses[index];
      if (res.type !== 'pause') {
        addChatVisualElement(chatBoxElement, res, isUser, isTop);
        ...
        // delay comes in seconds, so convert to ms
        var delayTime = res.time * 1000;
        setTimeout(function () {
  ...
  function sendMessage(text) {
    ...
    var context;
    var latestResponse = ConversationApi.getResponsePayload();
    if (latestResponse) {
      context = latestResponse.context;
    }

    // Send the user message
    ConversationApi.sendRequest(text, context);
  }
